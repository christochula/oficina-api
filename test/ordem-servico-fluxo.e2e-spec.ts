...
import { NOTIFICACAO_ORCAMENTO_GATEWAY } from '../src/ordem-servico/application/portas/notificacao-orcamento.gateway';
...
import { JWT_AUDIENCE, JWT_ISSUER, jwtSecret } from '../src/auth/jwt-config';

jest.setTimeout(120000);
const E2E_ADMIN_PASSWORD = 'E2eAdminPassword!2026';
...
function gerarCpfValido(seed: number): string {
  ...
  const calcDigito = (numeros: number[], pesoInicial: number): number => {
    const soma = numeros.reduce(
      (acc, n, idx) => acc + n * (pesoInicial - idx),
      0,
    );
...
function criarTokenCliente(clienteId: string): string {
  return jwt.sign(
    {
      role: 'CLIENTE',
...
describe('Fluxo completo da Ordem de Serviço (e2e com Testcontainers)', () => {
  ...
  beforeAll(async () => {
    ...
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(NOTIFICACAO_ORCAMENTO_GATEWAY)
      .useValue({ enviarParaAprovacao: jest.fn() })
      .compile();
    ...
    try {
      execSync('npx ts-node prisma/seed.ts', {
        env: {
          ...
          ADMIN_SEED_PASSWORD: E2E_ADMIN_PASSWORD,
        },
        stdio: 'pipe',
      });
    } catch (error) {
  ...
  it('deve executar fluxo ponta a ponta: abrir, aprovar, executar, finalizar e entregar', async () => {
    ...
    const adminSenha = E2E_ADMIN_PASSWORD;
    ...
    const tokenMecanico = unwrap<{ accessToken: string }>(
      loginMecanicoResp.body,
    ).accessToken;
    ...
    const tokenCliente = criarTokenCliente(clienteId);
  ...
  it('deve processar aprovacao externa via webhook com idempotencia e validar token', async () => {
    ...
    const tokenAdmin = unwrap<{ accessToken: string }>(
      loginAdminResp.body,
    ).accessToken;
    ...
    const webhookAprovacao = unwrap<{ status: string }>(
      webhookAprovacaoResp.body,
    );
    ...
    const webhookIdempotente = unwrap<{ status: string }>(
      webhookIdempotenteResp.body,
    );
