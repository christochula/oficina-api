...
import { paraRespostaOrdemServicoCliente } from './dtos/ordem-servico-cliente.dto';
...
export class OrdemServicoController {
  ...
  async atribuir(
    @Param('id') id: string,
    @Param('mecanicoId') mecanicoId: string,
  ) {
   ...
   * Papel permitido: CLIENTE titular da OS.
   */
  @Patch(':id/aprovar')
  @Version('1')
  ...
  async aprovar(@Param('id') id: string, @UsuarioAtual() usuario: JwtPayload) {
    const os = await this.aprovarOrcamento.executar(
      id,
      usuario.clienteId ?? usuario.sub,
    );
    return paraRespostaOrdemServicoCliente(os);
  }
  ...
  async rejeitar(@Param('id') id: string, @UsuarioAtual() usuario: JwtPayload) {
    const os = await this.rejeitarOrcamento.executar(
      id,
      usuario.clienteId ?? usuario.sub,
    );
    return paraRespostaOrdemServicoCliente(os);
  ...
  @Papeis(PapelUsuario.ADMINISTRADOR, PapelUsuario.CONSULTOR_TECNICO)
  @ApiOperation({
    summary: 'Tempo de ciclo personalizado com descontos opcionais',
  })
  async tempoCicloPersonalizado(
    ...
    const descontarArray = descontar
      ? Array.isArray(descontar)
        ? descontar
        : [descontar]
      : [];
    const osIds = osId ? (Array.isArray(osId) ? osId : [osId]) : undefined;
    return this.tempoCiclo.executar({
      eventoInicio,
      eventoFim,
      descontar: descontarArray,
      osIds,
    });
  }
  ...
  async listarMinhas(
    ...
    const resultado = await this.listarMinhasOS.executar({
      usuarioId: usuario.clienteId ?? usuario.sub,
      ...paginacao,
    });
    return {
      ...resultado,
      data: resultado.data.map(paraRespostaOrdemServicoCliente),
    };
  }
  ...
  async buscarMinha(
    ...
    const os = await this.buscarMinhaOS.executar(
      id,
      usuario.clienteId ?? usuario.sub,
    );
    return paraRespostaOrdemServicoCliente(os);
