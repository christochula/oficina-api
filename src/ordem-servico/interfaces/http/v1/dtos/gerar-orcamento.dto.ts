...
class LinhaServicoDto {
  @ApiProperty({ enum: TipoLinhaServico, description: 'MATERIAL (peça) ou SERVICO (mão de obra)' })
  ...
  @ApiPropertyOptional({ description: 'ID da peça do estoque (apenas para linhas MATERIAL)' })
  @IsOptional()
  @IsString()
  pecaId?: string;
}
...
class GrupoOrcamentoDto {
  @ApiProperty({ description: 'Título descritivo do grupo (ex: "Retífica do Motor")' })
  @IsString()
  titulo: string;

  @ApiProperty({ type: [LinhaServicoDto], description: 'Linhas de serviço do grupo (ao menos uma)' })
  @IsArray()
  @ArrayMinSize(1)
...
export class GerarOrcamentoDto {
  @ApiProperty({
    ...
    description: 'Grupos de itens do orçamento (ao menos um grupo com ao menos uma linha)',
  })
  @IsArray()
  @ArrayMinSize(1)
  @ValidateNested({ each: true })
  ...
  @ApiPropertyOptional({ description: 'Notas internas da equipe da oficina (não visíveis ao cliente)' })
  @IsOptional()
  @IsString()
  notasInternas?: string;

  @ApiPropertyOptional({ description: 'Notas enviadas ao cliente junto com o orçamento' })
  @IsOptional()
  @IsString()
  notasCliente?: string;
}
