...
const mockOsRepo = { buscarEntregues: jest.fn(), buscarPorId: jest.fn(), salvar: jest.fn() };
...
describe('RelatorioLeadTimeUseCase', () => {
  ...
  it('deve calcular média, mínimo e máximo corretamente', async () => {
    ...
    const os1 = criarOsEntregue(base, new Date(base.getTime() + 4 * 3600 * 1000)); // 4h
    const os2 = criarOsEntregue(base, new Date(base.getTime() + 8 * 3600 * 1000)); // 8h
    const os3 = criarOsEntregue(base, new Date(base.getTime() + 12 * 3600 * 1000)); // 12h
