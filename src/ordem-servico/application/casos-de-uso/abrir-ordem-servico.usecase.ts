...
export class AbrirOrdemServicoUseCase {
  ...
  async executar(input: AbrirOrdemServicoInput): Promise<OrdemServico> {
    ...
    const servicosSolicitados = await Promise.all(
      (input.servicosSolicitados ?? []).map(async (s) => {
        const servico = await this.servicoRepository.buscarPorId(
          ServicoOficinaId.de(s.servicoId),
        );
        if (!servico)
          throw new RecursoNaoEncontrado('Serviço do catálogo', s.servicoId);
        return {
    ...
    const pecasSolicitadas = await Promise.all(
      (input.pecasSolicitadas ?? []).map(async (p) => {
        ...
        if (!estoque)
          throw new RecursoNaoEncontrado('Peça do estoque', p.pecaId);
        return p;
      }),
    );
    ...
    const problemasRelatados =
      (input.problemasRelatados?.length ?? 0) > 0 ||
      (servicosSolicitados.length ?? 0) > 0
        ? input.problemasRelatados
        : resumoPecas
          ? [
              {
                descricao:
                  'Solicitação de peças registrada na abertura (ver notas internas)',
              },
            ]
          : input.problemasRelatados;
  ...
  private async resolverCliente(
    ...
    if (!input.cliente) {
      throw new RegraDeNegocio(
        'Informe clienteId ou dados do cliente para abertura da OS',
      );
    }
    ...
    const clienteExistente =
      await this.clienteRepository.buscarPorNumeroDoc(numeroDoc);
  ...
  private async resolverVeiculo(
    input: AbrirOrdemServicoInput,
  ): Promise<Veiculo> {
    if (input.veiculoId) {
      const veiculo = await this.veiculoRepository.buscarPorId(
        VeiculoId.de(input.veiculoId),
      );
    ...
    if (!input.veiculo) {
      throw new RegraDeNegocio(
        'Informe veiculoId ou dados do veículo para abertura da OS',
      );
    }
