...
function criarUseCase() {
  mockDatabaseTransaction.executar.mockImplementation(
...
function osEmExecucao() {
  ...
  os.iniciarExecucao(os.mecanicoResponsavelId!);
  return os;
}
...
describe('RegistrarConsumoPecaUseCase', () => {
  ...
  it('deve registrar consumo de peca e chamar EstoqueService dentro da transacao', async () => {
    ...
    await uc.executar({
      ...
      pecaId: 'pc_01',
      quantidade: 2,
      mecanicoId: os.mecanicoResponsavelId!,
    });

    expect(mockDatabaseTransaction.executar).toHaveBeenCalledTimes(1);
    expect(mockEstoqueService.processarConsumoPeca).toHaveBeenCalledWith(
      'pc_01',
      2,
    );
  });
  ...
  it('nao deve limpar eventos se a transacao falhar', async () => {
    const os = osEmExecucao();
    mockOsRepo.buscarPorId.mockResolvedValue(os);
    mockEstoqueService.processarConsumoPeca.mockRejectedValue(
      new Error('falha no estoque'),
    );
