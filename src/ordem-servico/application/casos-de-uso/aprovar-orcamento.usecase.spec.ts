...
function criarUseCase() {
  return new AprovarOrcamentoUseCase(mockOsRepo as any, mockBuscarClientePorUsuario as any);
}
...
describe('AprovarOrcamentoUseCase', () => {
  ...
  it('deve aprovar orcamento e mudar status para APROVADA', async () => {
  ...
  it('deve salvar OS apos aprovacao', async () => {
    const os = osAguardandoAprovacao();
    mockBuscarClientePorUsuario.executar.mockResolvedValue({ id: { valor: 'cl_01' } });
    mockOsRepo.buscarPorId.mockResolvedValue(os);
    mockOsRepo.salvar.mockResolvedValue(undefined);
  ...
  it('deve lancar AcessoNegado se o cliente nao for o titular da OS', async () => {
    const os = osAguardandoAprovacao();
    mockBuscarClientePorUsuario.executar.mockResolvedValue({ id: { valor: 'cl_outro' } });
    mockOsRepo.buscarPorId.mockResolvedValue(os);

    const uc = criarUseCase();
    await expect(uc.executar('os_01', 'us_cliente')).rejects.toThrow(AcessoNegado);
  });
});
