...
function criarUseCase() {
  return new BuscarStatusOrdemServicoPublicoUseCase(mockOsRepo as any, mockClienteRepo as any);
}
...
describe('BuscarStatusOrdemServicoPublicoUseCase', () => {
  ...
  it('deve retornar status quando OS pertencer ao cliente do documento', async () => {
    ...
    mockClienteRepo.buscarPorNumeroDoc.mockResolvedValue({ id: { valor: 'cl_01' } });
  ...
  it('deve lançar RegraDeNegocio quando número da OS for inválido', async () => {
    ...
    await expect(useCase.executar(-1, '11144477735')).rejects.toThrow(RegraDeNegocio);
  });

  it('deve lançar RecursoNaoEncontrado quando OS não pertencer ao cliente', async () => {
    mockClienteRepo.buscarPorNumeroDoc.mockResolvedValue({ id: { valor: 'cl_02' } });
    mockOsRepo.buscarPorNumero.mockResolvedValue({ clienteId: 'cl_01' });

    const useCase = criarUseCase();

    await expect(useCase.executar(123, '11144477735')).rejects.toThrow(RecursoNaoEncontrado);
  });
});
