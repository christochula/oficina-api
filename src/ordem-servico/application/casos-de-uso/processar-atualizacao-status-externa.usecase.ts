...
export class ProcessarAtualizacaoStatusExternaUseCase {
  ...
  async executar(
    input: ProcessarAtualizacaoStatusExternaInput,
    ...
    if (!novoStatus) {
      throw new RegraDeNegocio(
        `Status externo '${input.novoStatus}' nao reconhecido`,
      );
    }
    ...
    if (novoStatus === StatusOrdemServico.APROVADA) {
      ...
      os.atualizarStatusPorIntegracaoExterna(
        novoStatus,
        origem,
        input.idMensagemExterna,
      );
    }
