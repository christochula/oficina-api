...
export class ProcessarAprovacaoExternaOrcamentoUseCase {
  ...
  async executar(input: ProcessarAprovacaoExternaOrcamentoInput): Promise<OrdemServico> {
    ...
    if (input.decisao === 'APROVADO' && os.status === StatusOrdemServico.APROVADA) {
      return os;
    }

    if (input.decisao === 'RECUSADO' && os.status === StatusOrdemServico.CANCELADA) {
      return os;
    }
