...
describe('OrdemServico', () => {
  describe('abrir()', () => {
    ...
    it('deve aceitar OS com apenas serviços solicitados', () => {
      const os = OrdemServico.abrir({
        clienteId: 'cl_01',
        veiculoId: 've_01',
        servicosSolicitados: [{ servicoId: 'sv_01', nomeServico: 'Troca de pneus' }],
      });
      expect(os.status).toBe(StatusOrdemServico.RECEBIDA);
    });
  ...
  describe('atribuirMecanico()', () => {
    ...
    it('deve lançar RegraDeNegocio se OS não estiver no status RECEBIDA', () => {
      const os = osAtribuida();
      expect(() => os.atribuirMecanico(criarMecanico())).toThrow(RegraDeNegocio);
    });
  ...
  describe('iniciarDiagnostico()', () => {
    ...
    it('deve lançar RegraDeNegocio se OS não estiver ATRIBUIDA', () => {
      const os = osRecebida();
      expect(() => os.iniciarDiagnostico('us_01')).toThrow(RegraDeNegocio);
    });
  ...
  describe('registrarDiagnostico()', () => {
    ...
    it('deve lançar RegraDeNegocio se OS não estiver EM_DIAGNOSTICO', () => {
      const os = osAtribuida();
      expect(() => os.registrarDiagnostico('Diagnóstico', 'us_01')).toThrow(RegraDeNegocio);
    });
  });
  ...
  describe('gerarOrcamento()', () => {
    ...
    it('deve lançar RegraDeNegocio se OS não estiver em ATRIBUIDA ou EM_DIAGNOSTICO', () => {
      const os = osRecebida();
      expect(() => os.gerarOrcamento(grupos, 'us_01')).toThrow(RegraDeNegocio);
    });
  ...
  describe('registrarConsumoPeca()', () => {
    ...
    it('deve lançar RegraDeNegocio se OS não estiver EM_EXECUCAO', () => {
      const os = osAprovada();
      expect(() => os.registrarConsumoPeca('pc_01', 1, 'us_01')).toThrow(RegraDeNegocio);
    });

    it('deve lançar RegraDeNegocio se quantidade for zero', () => {
      const os = osEmExecucao();
      expect(() => os.registrarConsumoPeca('pc_01', 0, 'us_01')).toThrow(RegraDeNegocio);
    });
