...
import {
  GrupoOrcamento,
  GrupoOrcamentoInput,
} from './value-objects/grupo-orcamento.vo';
...
export class OrdemServico extends EntidadeBase<OrdemServicoId> {
   ...
   */
  static reconstituir(
    props: PropriedadesOrdemServico & { id: OrdemServicoId; numero: number },
  ): OrdemServico {
  ...
  atribuirMecanico(mecanico: Usuario): void {
    ...
    this.registrarHistorico(
      'MECANICO_ATRIBUIDO',
      `RECEBIDA → ATRIBUIDA | Mecânico ${mecanico.nome} atribuído`,
      mecanico.id.valor,
      anterior,
  ...
  gerarOrcamento(
    ...
    if (grupos.length === 0) {
      throw new RegraDeNegocio(
        'O orçamento deve ter ao menos um grupo de serviços',
      );
    }
    if (grupos.some((g) => g.linhas.length === 0)) {
      throw new RegraDeNegocio(
        'Cada grupo do orçamento deve ter ao menos uma linha de serviço',
      );
    }
  ...
  aprovarOrcamento(usuarioId: string): void {
    ...
    if (!this.orcamento)
      throw new RegraDeNegocio('Não existe orçamento para aprovar');
  ...
  rejeitarOrcamento(usuarioId: string): void {
    this.garantirStatus([StatusOrdemServico.AGUARDANDO_APROVACAO]);
    if (!this.orcamento)
      throw new RegraDeNegocio('Não existe orçamento para rejeitar');
  ...
  registrarConsumoPeca(
    ...
    if (quantidade <= 0)
      throw new RegraDeNegocio('Quantidade deve ser maior que zero');

    this.consumosPeca.push({ pecaId, quantidade });
    this._eventos.push(
      new ConsumoPecaEvento(this.id.valor, pecaId, quantidade),
    );
    // Sem mudança de status — statusAnterior === statusNovo === EM_EXECUCAO
    this.registrarHistorico(
      'PECA_CONSUMIDA',
      `Peça ${pecaId} (qtd: ${quantidade})`,
      mecanicoId,
      StatusOrdemServico.EM_EXECUCAO,
      StatusOrdemServico.EM_EXECUCAO,
    );
  ...
  atualizarStatusPorIntegracaoExterna(
    ...
    if (
      ...
      throw new RegraDeNegocio(
        `OS em status terminal '${this.status}' não aceita atualização externa`,
      );
    }
    ...
    if (indiceNovo < 0) {
      throw new RegraDeNegocio(
        `Status externo '${novoStatus}' não é aceito para sincronização`,
      );
    }
    ...
    const rastreio = idMensagemExterna
      ? ` | mensagem=${idMensagemExterna}`
      : '';
    this.registrarHistorico(
  ...
  private garantirMecanicoResponsavel(mecanicoId: string): void {
    if (
      !this.mecanicoResponsavelId ||
      this.mecanicoResponsavelId !== mecanicoId
    ) {
      throw new AcessoNegado(
        'Apenas o mecanico responsavel pode alterar esta ordem de servico',
      );
    }
  }
