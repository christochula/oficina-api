...
export class ExcecaoHttpFilter implements ExceptionFilter {
  constructor(
    private readonly logger: JsonLoggerService = new JsonLoggerService(),
  ) {}
   ...
   */
    ...
    const error =
      excecao instanceof Error ? excecao : new Error(String(excecao));
    const logPayload = {
      event: 'http_request_failed',
      method: requisicao.method,
      path: sanitizeHttpPath(requisicao.path),
      status_code: status,
      error_type: error.name,
    };

    if (status >= 500) {
      this.logger.error(logPayload, undefined, ExcecaoHttpFilter.name);
    } else {
      this.logger.warn(logPayload, ExcecaoHttpFilter.name);
    }
    ...
    resposta.status(status).json({
      ...
      mensagem,
      statusCode: status,
      caminho: sanitizeHttpPath(requisicao.path),
