...
export class Estoque extends EntidadeBase<PecaId> {
  ...
  darEntrada(quantidade: number): void {
    if (quantidade <= 0) throw new RegraDeNegocio('Quantidade de entrada deve ser maior que zero');
   ...
   */
  atualizarDadosPeca(dados: { nome?: string; descricao?: string | null; precoVenda?: number }): void {
    if (dados.nome !== undefined) this.peca.nome = dados.nome;
    if (Object.prototype.hasOwnProperty.call(dados, 'descricao')) this.peca.descricao = dados.descricao;
