...
export class PrismaVeiculoRepository implements VeiculoRepository {
   ...
   */
  async listar(pagina: number, porPagina: number): Promise<{ itens: Veiculo[]; total: number }> {
   ...
   */
  private mapear(r: {
    id: string; placa: string; renavam: string; chassi: string; marca: string;
    modelo: string; ano: number; cor: string; quilometragem: number; ativo: boolean;
