...
describe('CriarUsuarioUseCase', () => {
  ...
  it('deve lançar ConflitoDeRecurso se email já existir', async () => {
    ...
    await expect(
      uc.executar({ nome: 'João', email: 'joao@test.com', senha: '123', papel: PapelUsuario.MECANICO }),
  ...
  it('deve salvar o usuário criado', async () => {
    ...
    const uc = criarUseCase();
    await uc.executar({ nome: 'Ana', email: 'ana@test.com', senha: 'pass', papel: PapelUsuario.ADMINISTRADOR });
