...
export class ClienteController {
  ...
  async listar(@Query() paginacao: PaginacaoDto): Promise<RespostaPaginadaDto<Cliente>> {
    const { itens, total } = await this.listarClientes.executar(paginacao.pagina, paginacao.porPagina);
  ...
  @Patch(':id')
  @Version('1')
  @ApiOperation({ summary: 'Atualizar dados do cliente (exceto tipoDoc e numeroDoc)' })
  async atualizar(
    @Param('id') id: string,
