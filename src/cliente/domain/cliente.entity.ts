...
export class Cliente extends EntidadeBase<ClienteId> {
   ...
   */
  static criar(
    props: Omit<PropriedadesCliente, 'id' | 'criadoEm' | 'atualizadoEm'>,
  ): Cliente {
    const documentoValido =
      props.tipoDoc === TipoDocumento.CPF
        ? isValidCPF(props.numeroDoc)
        : isValidCNPJ(props.numeroDoc);
    if (!documentoValido) {
    ...
    const numeroDoc =
      props.tipoDoc === TipoDocumento.CPF
        ? cleanCPF(props.numeroDoc)
        : cleanCNPJ(props.numeroDoc);
    return new Cliente({ ...props, numeroDoc });
  }
  ...
  static reconstituir(
    props: Required<
      Pick<PropriedadesCliente, 'id' | 'criadoEm' | 'atualizadoEm'>
    > &
      PropriedadesCliente,
  ): Cliente {
