...
/**
 * Caso de uso que reativa um serviço do catálogo.
 * Serviços reativados voltam a aparecer na listagem e podem ser selecionados em novas OS.
 * Apenas ADMINISTRADOR pode executar este caso de uso.
 */
@Injectable()
export class AtivarServicoOficinaUseCase {
  constructor(
  ...
  async executar(id: string): Promise<ServicoOficina> {
    ...
    if (servico.ativo) {
      throw new RegraDeNegocio('Serviço já está ativo');
    }
